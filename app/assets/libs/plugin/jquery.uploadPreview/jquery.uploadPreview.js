          ...
          $(settings.input_field).change(function() {
            ...
            if (files.length > 0) {
              ...
              reader.addEventListener("load",function(event) {
				...
				//   $(settings.preview_box).css("height", settings.box_width);
				 // $(settings.preview_box).css("width", settings.box_height);
				  // $(settings.preview_box).css("padding-top", "66,64%");
				 
				   $(settings.preview_box).css("background-repeat", "no-repeat");	
				   $(settings.preview_box).css({"color":"#ccc","background-color":settings.box_bg,"text-align":"center"});	
                  ...
                  $(settings.preview_box).html("<audio controls><source src='" + loadedFile.result + "' type='" + file.type + "' />Your browser does not support the audio element.</audio>");
					...
					// $(settings.preview_box).html("<video controls width=\"100%\" height=\"100%\"> <source src='" + loadedFile.result + "' type='" + file.type + "'></video>");				
					$("#video-label").html('<div style="color:#000;padding-top:50px;">'+file.name+'</div>');
                } else {
