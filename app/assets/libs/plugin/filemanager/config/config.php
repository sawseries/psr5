...
/*
$dirroot = explode("/", dirname(__FILE__));
$dirpath = "";
$countdir = 3;
foreach ($dirroot as $dir){
	if(count($dirroot) > $countdir)
		$dirpath.=$dir."/";
	$countdir = $countdir+1;
}
include ($dirpath."class/config.php");
*/
mb_internal_encoding('UTF-8');
//------------------------------------------------------------------------------
...
//$upload_dir = '/show/files/'.$_PROFILE_CODE.'/'; // path from base_url to base of upload folder (with start and final /)
$upload_dir = '/clients/meshowtime/show/files/'.$_PROFILE_CODE.'/'; // path from base_url to base of upload folder (with start and final /)
$current_path = '../../../show/files/'.$_PROFILE_CODE.'/'; // relative path from filemanager folder to upload folder (with final /)
$thumbs_base_path = '../../../show/files-thumbs/'.$_PROFILE_CODE.'/'; // relative path from filemanager folder to thumbs folder (with final /)
